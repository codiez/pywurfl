...
from pywurfl.algorithms.wurfl.normalizers import default_normalizer
...
class Devices(object):
    ...
    def select_ua(self, devua, actual_device_root=False, normalize=True,
        ...
        @param actual_device_root: Return a device that is an actual device
                                   root
        @type actual_device_root: boolean
        @param normalize: Normalize the user agent by removing extraneous text.
        ...
        if device is None:
            if normalize:
                devua = default_normalizer(devua)
    ...
    def md5_hexdigest(self):
        ...
        return md5.new(u''.join(data)).hexdigest()
