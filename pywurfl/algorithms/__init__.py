...
class TwoStepAnalysis(Algorithm):
    """
    WURFL Two Step Analysis algorithm based on the Java API implementation
    """
    def __init__(self, devices):
        for ua in devices.uas:
            for h in handlers:
                if h.can_handle(ua):
                    h.add(ua, devices.devuas[ua].devid)
                    break

    def _determine_handler(self, ua):
        for handler in handlers:
            if handler.can_handle(ua):
                #print "Handling with %s" % handler.__class__
                return handler

    def __call__(self, ua, devices=None):
        handler = self._determine_handler(ua)
        match = handler(ua)
        return devices.devids[match]
